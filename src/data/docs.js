...
Rho API
  ...
  +-- Google OAuth`),
    ...
    '/docs/getting-started': {
        ...
        summary: 'Connect a Vue, React, Nuxt, Next.js, or plain TypeScript application to a Rho API.',
        sections: [
            {
                id: 'install',
                title: '1. Install the SDK',
                body: ['Install the published client package in the frontend application.'],
                code: 'npm install @rho-commerce/sdk'
            },
            {
                id: 'client',
                title: '2. Create the client',
                body: ['The only required client option is baseURL. Remove a trailing slash from the URL automatically; the SDK already normalizes it.'],
...
export const rho = new RhoClient({
  baseURL: import.meta.env.VITE_RHO_API_URL
            ...
            {
                id: 'environment',
                title: '3. Configure the API URL',
                body: [
                    'The API base URL is public configuration. In a Vite application, a VITE_* variable is appropriate because it is intentionally exposed to the browser.',
                    'Do not put database credentials, JWT secrets, payment secret keys, or OAuth client secrets in frontend environment variables.'
                ],
                code: 'VITE_RHO_API_URL=https://api.example.com'
            },
            {
                id: 'first-request',
                title: '4. Make a public request',
                body: ['Products and categories can be loaded without a customer token. The SDK returns camelCase properties even though the HTTP API uses snake_case.'],
                code: code(`const products = await rho.storefront.products({
  page: 1,
  limit: 24
})
...
onMounted(async () => {
  try {
    const result = await rho.storefront.products({ page: 1, limit: 24 })
...
export function Products() {
  ...
  useEffect(() => {
    rho.storefront.products({ page: 1, limit: 24 })
  ...
  return products.map(product => (
    <article key={product.id}>{product.name}</article>
            ...
            {
                id: 'auth-first',
                title: '5. Add authentication',
                body: ['After public catalogue reads work, add signup or login. Authentication responses contain both accessToken and refreshToken plus the user object.'],
...
rho.setToken(result.accessToken)`)
            },
            {
                ...
                list: [
                    'API URL comes from frontend configuration.',
                    'The SDK client is created once per application/service boundary.',
                    'Protected requests receive the current access token.',
                    '401, 403, validation, network, loading, and empty states have UI handling.',
                    'Checkout totals come from Rho rather than a client-side calculation.',
                    'Payment return pages verify server state before displaying a final success state.'
                ]
            }
        ]
    },
...
const orderId = checkout.orderId`),
    ...
    '/docs/orders': {
        title: 'Orders',
        eyebrow: 'Order UI',
        summary: 'Render order confirmation and detail screens from the order record returned by Rho.',
        sections: [
            {
                id: 'creation',
                title: 'Orders come from checkout',
                body: ['For the customer purchase flow, checkout.create() is the normal way to create an order. Direct orders.create() exists for admin use and accepts a full order payload, including totals and items.'],
                code: code(`const checkout = await rho.checkout.create({
  discountCode: couponCode || undefined
})

const order = await rho.orders.get(checkout.orderId)`)
            },
            {
                id: 'get',
                title: 'Load an order',
...
console.log(order.items)`)
            ...
            {
                id: 'items',
                title: 'Render order items from snapshots',
                body: ['OrderItem contains name, sku, quantity, unitPrice, and totalPrice. Use these values for historical order display instead of fetching the current product and assuming its price is unchanged.'],
                code: code(`order.items.map(item => ({
  name: item.name,
  sku: item.sku,
  ...
  total: item.totalPrice
            ...
            {
                id: 'history',
                title: 'Order history limitation',
                body: ['The current SDK exposes orders.get(id), not a customer order-list method. Do not document rho.orders.list() as if it exists. If your account page needs order history, the backend contract needs a list endpoint or your deployment needs another supported data source.']
            },
            {
                id: 'status-admin',
                title: 'Admin status UI',
                body: ['Admin applications can update an order status. Customer applications should treat status as read-only.'],
